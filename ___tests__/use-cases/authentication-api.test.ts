...
// Fix: use string literals instead of enum values that can't be used as values
const MOCK_AUTHENTICATION_MODES = [
  {
    type: 'internal', // Instead of AuthenticationModeType.INTERNAL
    internal: {
      enabled: true
  ...
  {
    type: 'openid', // Instead of AuthenticationModeType.OPENID
    internal: undefined,
    openid: {
      id: 'google'
...
describe('AuthenticationApiDataSourceImpl', () => {
  ...
  describe('login', () => {
    ...
    it('should throw error when token is not provided', async () => {
      ...
      const dataSource = new AuthenticationApiDataSourceImpl();
      await expect(dataSource.login(MOCK_LOGIN_REQUEST)).rejects.toThrow('Invalid credentials');
    });
  });
  ...
  describe('getOAuthUrl', () => {
    ...
    it('should throw error when redirect URI is not provided', async () => {
      ...
      const dataSource = new AuthenticationApiDataSourceImpl();
      await expect(dataSource.getOAuthUrl('google')).rejects.toThrow('No redirect URL provided');
    });
  });
  ...
  describe('handleOAuthRedirect', () => {
    it('should successfully handle OAuth redirect', async () => {
      ...
      const dataSource = new AuthenticationApiDataSourceImpl();
      const result = await dataSource.handleOAuthRedirect(MOCK_OAUTH_REDIRECT_REQUEST);
    ...
    it('should throw error when token is not provided', async () => {
      ...
      const dataSource = new AuthenticationApiDataSourceImpl();
      await expect(dataSource.handleOAuthRedirect(MOCK_OAUTH_REDIRECT_REQUEST)).rejects.toThrow('No token received from OAuth redirect');
    });
  });
